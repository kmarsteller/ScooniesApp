...
function findRegionMaxRounds(regionPicks, allRegionTeams) {
    ...
    function search(slots, round) {
        ...
        return pathScore(totalA) >= pathScore(totalB) ? totalA : totalB;
    }
...
function calcMaxScore(picks, allTeams, ffMatchups) {
    ...
    picks.forEach(pick => {
        ...
        // Find the pick that blocks this one (same region takes priority over SF bracket)
        const sameRegionBlocker = picks.find(p =>
            p.team_name !== pick.team_name && !p.is_eliminated &&
            p.region === pick.region &&
            (maxRounds.get(p.team_name) || 0) > currentRound
        );
        const sfBlocker = !sameRegionBlocker && picks.find(p =>
            p.team_name !== pick.team_name && !p.is_eliminated &&
            p.region !== pick.region &&
            sfBracket[p.region] === sfBracket[pick.region] &&
            (maxRounds.get(p.team_name) || 0) >= 5
        );
        const blocker = sameRegionBlocker || sfBlocker;
        ...
        if (blocker) {
            blocked.push({
                ...
                roundName: sameRegionBlocker
                    ? (ROUND_NAMES[currentRound + 1] || `Round ${currentRound + 1}`)
                    : 'Final Four',
            });
        }
    });
